...
app.post(
  ...
  [
    body("name")
    .trim().notEmpty()
    .withMessage("Please enter ur name")
    .custom(async (value) => {
        const isDuplicateName = checkDuplicate(value);
        if(isDuplicateName) {
...
app.get("/contact/delete-contact/:name", (req, res) => {
  ...
  if (!contact) {
    ...
    console.log(
      chalk.bgRed.inverse(`contact ${req.params.name} has been deleted`)
    );
...
app.post(
  ...
  (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      console.log(
        chalk.bgYellow.inverse(`contact ${req.body.name} has been updated`)
      );
      res.redirect("/contact");
    }
  }
